...
let metarIcao = ""; // Inicializa a variável

try {
    const metarResponse = await fetch(metarUrl);
    const metarData = await metarResponse.json();

    // Acessa o METAR corretamente
    metarIcao = metarData.data?.data?.[0]?.mens || " ";
} catch (metarError) {
    console.warn("Erro ao obter METAR:", metarError);
    metarIcao = "erro";
}
...
// Corrigido para usar a tag correta
const runwayIdent = xmlDoc.querySelector("runways > runway > ident") ? xmlDoc.querySelector("runways > runway > ident").textContent : "Identificador de pista não encontrado";



// Adiciona a informação do runway ao texto final
const resultTextWithRunway = `${cidade}-${estado} (${fir}) RWY ${runwayIdent}`;

// Aqui usamos 'resultTextWithRunway' em vez de 'resultText'
const resultElement = document.createElement("span");
resultElement.innerHTML = resultTextWithRunway;
...
const metarText = `<br>${metarIcao}`;
const metarElement = document.createElement("span");
metarElement.innerHTML = metarText;
resultElement.appendChild(metarElement);
